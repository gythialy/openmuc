...
public class ChannelResourceServlet extends GenericServlet {
            ...
            if (pathInfoArray.length < 1) {
                ...
                if (channelConfig != null) {
                    ...
                    if (pathInfoArray.length == 2 && pathInfoArray[1].equalsIgnoreCase(Const.CONFIGS)) {
                        ...
                        ServletLib.sendHTTPErrorAndLogDebug(response, HttpServletResponse.SC_NOT_FOUND, logger,
                                                            "Requested rest path is not available.", " Rest Path = ",
                                                            request.getPathInfo());
                    }
                } else {
                    ServletLib.sendHTTPErrorAndLogDebug(response, HttpServletResponse.SC_NOT_FOUND, logger,
                ...
                try {
                    ...
                    if (rootConfig.getDriver(channelId) == null) {
                        ...
                        ServletLib.sendHTTPErrorAndLogErr(response, HttpServletResponse.SC_INTERNAL_SERVER_ERROR, logger,
                                                          "Not able to delete channel ", channelId);
                    }
    ...
    private void doGetConfigField(ToJson json, String channelID, String configField, HttpServletResponse response) throws IOException {
    ...
    private boolean setAndWriteChannelConfig(String channelID, HttpServletResponse response, FromJson json, boolean isHTTPPut) {
        ...
        try {
            ...
            ServletLib.sendHTTPErrorAndLogErr(response, HttpServletResponse.SC_CONFLICT, logger, "Could not write channel \"", channelID,
                                              "\".");
            e.printStackTrace();
    ...
    private boolean setAndWriteHttpPutChannelConfig(String channelID, HttpServletResponse response, FromJson json) throws
            JsonSyntaxException, ConfigWriteException, RestConfigIsNotCorrectException, MissingJsonObjectException, IllegalStateException {
    ...
    private boolean setAndWriteHttpPostChannelConfig(String channelID, HttpServletResponse response, FromJson json) throws
            JsonSyntaxException, ConfigWriteException, RestConfigIsNotCorrectException, Error, MissingJsonObjectException,
    ...
    private void doSetRecord(String channelID, HttpServletResponse response, FromJson json) throws ClassCastException {
        ...
        if (record.getFlag() == null) {
            ServletLib.sendHTTPErrorAndLogDebug(response, HttpServletResponse.SC_NOT_ACCEPTABLE, logger, "No flag setted.");
        } else if (record.getValue() == null) {
            ServletLib.sendHTTPErrorAndLogDebug(response, HttpServletResponse.SC_NOT_ACCEPTABLE, logger, "No value setted.");
        } else {
