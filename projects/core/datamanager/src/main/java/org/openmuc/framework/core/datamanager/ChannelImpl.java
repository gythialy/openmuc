            ...
            try {
                switch (config.valueType) {
                    case BOOLEAN:
                        ...
                        break;
                    case BYTE:
                        convertedRecord = new Record(new ByteValue(record.getValue().asByte()), record.getTimestamp(), record.getFlag());
                        break;
                    case SHORT:
                        convertedRecord = new Record(new ShortValue(record.getValue().asShort()), record.getTimestamp(), record.getFlag());
                        break;
                    case INTEGER:
                        convertedRecord = new Record(new IntValue(record.getValue().asInt()), record.getTimestamp(), record.getFlag());
                        break;
                    case LONG:
                        convertedRecord = new Record(new LongValue(record.getValue().asLong()), record.getTimestamp(), record.getFlag());
                        break;
                    case FLOAT:
                        convertedRecord = new Record(new FloatValue(record.getValue().asFloat()), record.getTimestamp(), record.getFlag());
                        break;
                    case DOUBLE:
                        ...
                        break;
                    case BYTE_ARRAY:
                        convertedRecord = new Record(new ByteArrayValue(record.getValue().asByteArray()), record.getTimestamp(),
                                                     record.getFlag());
                        break;
                    case STRING:
