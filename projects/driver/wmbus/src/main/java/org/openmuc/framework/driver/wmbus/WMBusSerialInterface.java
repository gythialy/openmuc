...
public class WMBusSerialInterface {
    ...
    private final static Map<String, WMBusSerialInterface> interfaces = new HashMap<String, WMBusSerialInterface>();
    private final HashMap<Integer, WMBusConnection> connectionsBySecondaryAddress = new HashMap<Integer, WMBusConnection>();
    ...
    public class Receiver implements WMBusListener {
        ...
        public void discardedBytes(byte[] bytes) {
            if (logger.isDebugEnabled()) {
                logger.debug("received bytes that will be discarded: " + HexConverter.getHexStringFromByteArray(bytes));
            }
        }
        ...
        public void newMessage(WMBusDataMessage message) {
            ...
            synchronized (this) {
                WMBusConnection connection = connectionsBySecondaryAddress.get(message.getSecondaryAddress().getHashCode());

                if (connection == null) {
                    if (logger.isTraceEnabled()) {
                        logger.trace("WMBus: connection is null, from device: {} with HashCode: {}",
                                     message.getSecondaryAddress().getDeviceId().toString(), message.getSecondaryAddress().getHashCode());
                    }
                    return;
                }
                ...
                if (channelContainers.size() == 0) {
                    if (logger.isTraceEnabled()) {
                        logger.trace("WMBus: channelContainers.size == 0, from device: " + message.getSecondaryAddress().getDeviceId()
                                                                                                  .toString());
                    }
                    return;
                }
                ...
                try {
                    ...
                    if (logger.isWarnEnabled()) {
                        logger.warn("Unable to decode header of variable data response or received message: {}", message, e);
                    }
                    return;
                }
                ...
                for (DataRecord dataRecord : dataRecords) {
                    dibvibs[i++] = HexConverter.getShortHexStringFromByteArray(dataRecord.getDIB()) + ':' + HexConverter
                            .getShortHexStringFromByteArray(dataRecord.getVIB());
                }

                List<ChannelRecordContainer> containersReceived = new ArrayList<ChannelRecordContainer>();
                    ...
                    for (DataRecord dataRecord : dataRecords) {
                        ...
                        if (dibvibs[i++].equalsIgnoreCase(container.getChannelAddress())) {
                            ...
                            try {
                                ...
                                if (logger.isWarnEnabled()) {
                                    logger.warn("Unable to parse received data record with <dib>:<vib> = " + dibvibs[i], e);
                                }
                                continue;
                            }
                            Value value = null;
                            switch (dataRecord.getDataValueType()) {
                                ...
                                case BCD:
                                    if (dataRecord.getMultiplierExponent() == 0) {
                                        ...
                                        value = new DoubleValue(((Bcd) dataRecord.getDataValue()).longValue() * Math
                                                .pow(10, dataRecord.getMultiplierExponent()));
                                        container.setRecord(new Record(value, timestamp));
                                    }
                                    break;
                                case NONE:
                            ...
                            if (logger.isTraceEnabled()) {
                                logger.trace("WMBus: Value from channel {}", container.getChannel().getId() + " is:" + value.toString());
                            }
    ...
    public static WMBusSerialInterface getInstance(String serialPortName, String transceiverString, String modeString) throws
        ...
        synchronized (interfaces) {
            ...
            if (serialInterface != null) {
                if (!serialInterface.modeString.equals(modeString) || !serialInterface.transceiverString.equals(transceiverString)) {
    ...
    private WMBusSerialInterface(String serialPortName, String transceiverString, String modeString) throws ArgumentSyntaxException, ConnectionException {
        ...
        if (modeString.equalsIgnoreCase("s")) {
            ...
            throw new ArgumentSyntaxException(
                    "The wireless M-Bus mode is not correctly specified in the device's parameters string. Should be S or T but is: " + modeString);
        }
        ...
        if (transceiverString.equals("amber")) {
            ...
            throw new ArgumentSyntaxException(
                    "The type of transceiver is not correctly specified in the device's parameters string. Should be amber or rc but is: " + transceiverString);
        }
    ...
    public Connection connect(SecondaryAddress secondaryAddress, String keyString) throws ArgumentSyntaxException {
        ...
        if (logger.isTraceEnabled()) {
            logger.trace("WMBus: connect device with ID " + secondaryAddress.getDeviceId().toString() + " and HashCode " + secondaryAddress
                    .getHashCode());
        }
