...
public class Aggregator implements DriverService, Connection {
    ...
    public DriverInfo getInfo() {
        ...
        String description = "Is able to aggregate logged values of a channel and writes the aggregated value into a new channel. " +
                "Different aggregation types supported.";
        String deviceAddressSyntax = "not needed";
        String parametersSyntax = "not needed";
        String channelAddressSyntax = "<id of channel which should be aggregated>:<type>[:<optionalLongSetting>] + \n Supported types: "
                + EAggregationType
                .getSupportedValues();
    ...
    public Object read(List<ChannelRecordContainer> containers, Object containerListHandle, String samplingGroup) throws
        ...
        for (ChannelRecordContainer container : containers) {
            ...
            try {
                ...
                logger.error("AggregationException occurred " + e.getMessage());
                setRecordWithErrorFlag(container, currentTimestamp);
            }
     ...
     */
    private void checkIntervals(long sourceLoggingInterval, long aggregationInterval, long aggregationSamplingTimeOffset) throws
    ...
    private void checkNumberOfRecords(List<Record> records, long sourceSamplingInterval, long aggregationInterval) throws
            AggregationException {
        ...
        if (records.size() != expectedNumberOfRecords) {
            throw new AggregationException("Expected " + expectedNumberOfRecords + " historical records but received " + records.size());
        }

    }

    private void checkIfChannelsNotNull(Channel sourceChannel, Channel aggregatedChannel) throws ChannelNotAccessibleException {
    ...
    private double performAggregation(Channel sourceChannel, List<Record> recordList, AggregatorChannelAddress address, Channel
        ...
        switch (address.getAggregationType()) {
            ...
            default:
                ...
                throw new WrongChannelAddressFormatException(
                        "Aggregation type: " + address.getAggregationType().name() + " not implemented yet");
        }
