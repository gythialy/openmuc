...
/*
	...
	Ok, I got rid of all those decorated names...
*/
...
//#endif    
    ...
    static void rBenchmark(libno.daveConnection dc, int bmArea) {
	...
	areaNumber=0; 
	if(bmArea==libno.daveDB) areaNumber=1;
	...
	for (i=1;i<101;i++) {
	    ...
	    p.addVarToReadRequest(libno.daveInputs,0,0,6);
	    p.addVarToReadRequest(libno.daveFlags,0,0,6);
	    p.addVarToReadRequest(libno.daveFlags,0,6,6);
	    p.addVarToReadRequest(bmArea,areaNumber,4,54);
	    p.addVarToReadRequest(bmArea,areaNumber,4,4);
	    res=dc.execReadRequest(p, rs);
	    if (res!=0) Console.WriteLine("\nerror "+res+" = "+libno.daveStrerror(res));
	    if (i%10==0) {
	        Console.Write("..."+i);		
	    }
	}	
