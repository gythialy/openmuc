...
public final class Iec62056Driver implements DriverService {
    ...
    private final static DriverInfo info = new DriverInfo("iec62056p21", // id
                                                          ...
                                                          "Synopsis: <serial_port> [-e] [-d <baud_rate_change_delay>]\nExamples for " +
                                                                  "<serial_port>: /dev/ttyS0 (Unix), COM1 (Windows)\n-e = enable handling" +
                                                                  " of echos caused by optical tranceivers\n-d <baud_rate_change_delay> =" +
                                                                  " delay of baud rate change in ms. Default is 0. USB to serial " +
                                                                  "converters often require a delay of up to 250ms.");
    ...
    public void scanForDevices(String settings, DriverDeviceScanListener listener) throws UnsupportedOperationException,
        ...
        if (args.length < 1 || args.length > 4) {
            throw new ArgumentSyntaxException("Less than one or more than four arguments in the settings are not allowed.");
        }
        ...
        try {
            ...
            listener.deviceFound(new DeviceScanInfo(serialPortName, deviceSettings, dataSets.get(0).getId().replaceAll("\\p{Cntrl}", "")));
    ...
    @Override
    public org.openmuc.framework.driver.spi.Connection connect(String deviceAddress, String settings) throws ArgumentSyntaxException,
            ConnectionException {
        ...
        try {
            ...
            connection.close();
            throw new ConnectionException("IOException trying to read meter: " + deviceAddress + ": " + e.getMessage(), e);
        } catch (TimeoutException e) {
            e.printStackTrace();
