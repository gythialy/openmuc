...
public final class Iec61850Driver implements DriverService {
    ...
    private final static DriverInfo info = new DriverInfo("iec61850", // id
                                                          ...
                                                          // parameters
                                                          "Synopsis: [-a <authentication_parameter>] [-lt <local_t-selector>] [-rt " +
                                                                  "<remote_t-selector>]",
                                                          // channel address
        ...
        if (!settings.isEmpty()) {
            ...
            for (int i = 0; i < args.length; i++) {
                if (args[i].equals("-a")) {
                    i++;
                    if (i == args.length) {
                        throw new ArgumentSyntaxException("No authentication parameter was specified after the -a parameter");
                    }
