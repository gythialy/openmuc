...
public final class Iec61850Connection implements Connection {
    ...
    public List<ChannelScanInfo> scanForChannels(String settings) throws UnsupportedOperationException, ConnectionException {
        ...
        for (BasicDataAttribute bda : bdas) {
            ...
            switch (bda.getBasicType()) {
                ...
                case UNICODE_STRING:
                    bda.setDefault();
                    scanInfos.add(new ChannelScanInfo(channelAddress, "", ValueType.BYTE_ARRAY, ((BdaBitString) bda).getValue().length));
                    break;
                case TIMESTAMP:
    ...
    public Object read(List<ChannelRecordContainer> containers, Object containerListHandle, String samplingGroup) throws
        ...
        for (ChannelRecordContainer container : containers) {
            ...
            if (container.getChannelHandle() == null) {
                ...
                try {
                    ...
                    logger.debug("ModelNode with object reference {} was found in the server model but is not a Basic Data Attribute.",
                                 container.getChannelAddress());
                    container.setRecord(new Record(Flag.DRIVER_ERROR_CHANNEL_WITH_THIS_ADDRESS_NOT_FOUND));
                    continue;
                }
        ...
        if (!samplingGroup.isEmpty()) {
            ...
            if (containerListHandle != null) {
                ...
                if (modelNode == null) {
                    logger.debug(
                            "Error reading sampling group: no FCDO/DA or DataSet with object reference {} was not found in the server " +
                                    "model.",
                            samplingGroup);
                    for (ChannelRecordContainer container : containers) {
                        container.setRecord(new Record(Flag.DRIVER_ERROR_SAMPLING_GROUP_NOT_FOUND));
                ...
                try {
                    ...
                    logger.debug(
                            "Error reading channel: ModelNode with sampling group reference {} was found in the server model but is not a" +
                                    " FcModelNode.",
                            samplingGroup);
                    for (ChannelRecordContainer container : containers) {
                        container.setRecord(new Record(Flag.DRIVER_ERROR_SAMPLING_GROUP_NOT_FOUND));
    ...
    public Object write(List<ChannelValueContainer> containers, Object containerListHandle) throws UnsupportedOperationException,
        ...
        for (FcModelNode fcModelNode : fcNodesToBeRequested) {
            try {
                ...
                logger.debug("Error writing to channel: service error calling setDataValues on {}: {}", fcModelNode.getReference(), e);
                for (BasicDataAttribute bda : fcModelNode.getBasicDataAttributes()) {
    ...
    void fillRequestedNodes(List<FcModelNode> fcNodesToBeRequested, List<FcModelNode> remainingFcModelNodes, ServerModel serverModel) {
    ...
    boolean checkParent(ModelNode modelNode, List<FcModelNode> fcNodesToBeRequested, List<FcModelNode> remainingModelNodes, ServerModel
            serverModel) {
    ...
    private void setFcModelNode(ChannelValueContainer container, FcModelNode fcModelNode) {
        if (fcModelNode instanceof BasicDataAttribute) {
            ...
            if (bdaValues.length != bdas.size()) {
                throw new IllegalStateException("attempt to write array " + valueString + " into fcModelNode " + fcModelNode
                        .getName() + " failed as the dimensions don't fit.");
            }
