...
public class LogFileWriter {
     ...
     */
    public void log(LogIntervalContainerGroup group, int loggingInterval, int logTimeOffset, Date date, HashMap<String, LogChannel>
            logChannelList) {
    ...
    private String setLoggingStringBuilder(List<LogRecordContainer> logRecordContainer, HashMap<String, LogChannel> logChannelList, Date
            date) {
            ...
            if (logRecordContainer.get(i).getRecord() != null) {
                if (logRecordContainer.get(i).getRecord().getFlag() == Flag.VALID) {
                    if (logRecordContainer.get(i).getRecord().getValue() == null) {
                        ...
                        ValueType valueType = logChannelList.get(logRecordContainer.get(i).getChannelId()).getValueType();
                        ...
                        switch (valueType) {
                            ...
                            case FLOAT:
                                ...
                                try {
                                    value = IESDataFormatUtils
                                            .convertDoubleToStringWithMaxLength(logRecordContainer.get(i).getRecord().getValue().asDouble(),
                                                                                size);
                            ...
                            case BYTE_ARRAY:
                                ...
                                if (byteArray.length > size) {
                                    value = LoggerUtils.buildError(Flag.UNKNOWN_ERROR);
                                    logger.error(
                                            "The byte array is too big, length is " + byteArray.length + " but max. length allowed is " +
                                                    size + ", ChannelId: " + logRecordContainer
                                                    .get(i).getChannelId());
                                } else {
                            ...
                            case STRING:
                                ...
                                if (value.length() > size) {
                                    value = LoggerUtils.buildError(Flag.UNKNOWN_ERROR);
                                    logger.error("The string is too big, length is " + value
                                            .length() + " but max. length allowed is " + size + ", ChannelId: " + logRecordContainer.get(i)
                                                                                                                                    .getChannelId());
                                }
                                break;
                            case BYTE:
    ...
    private void checkStringValue(String value) throws WrongCharacterException {
        ...
        if (value.contains(Const.SEPARATOR)) {
            throw new WrongCharacterException("Wrong character: String contains Seperator character: " + Const.SEPARATOR);
     ...
     */
    private PrintStream getStream(LogIntervalContainerGroup group, int loggingInterval, int logTimeOffset, Date date, HashMap<String, LogChannel> logChannelList) {
        ...
        try {
            if (file.exists()) {
                ...
                out = new PrintStream(new FileOutputStream(file, true), false, Const.CHAR_SET);
                String headerString = header.getIESDataFormatHeaderString(group, file.getName(), loggingInterval, logChannelList);
