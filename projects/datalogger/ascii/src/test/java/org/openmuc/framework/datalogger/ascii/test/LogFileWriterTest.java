...
public class LogFileWriterTest {
    ...
    static String testStringValueIncorrectASCII = "qwertzuiopü+asdfghjklöä#<yxcvbnm,.-^1234567890ß °!§$%&/()" +
            "=?QWERTZUIOPÜ*ASDFGHJKLÖÄ'>YXCVBNM;:_"; // 94
    // Zeichen
    ...
    public static void setup() {
        ...
        LogChannelTestImpl ch1 = new LogChannelTestImpl(ch01, "dummy description", dummy, ValueType.FLOAT, loggingInterval,
                                                        loggingTimeOffset);
        LogChannelTestImpl ch2 = new LogChannelTestImpl(ch02, "dummy description", dummy, ValueType.DOUBLE, loggingInterval,
                                                        loggingTimeOffset);
        LogChannelTestImpl ch3 = new LogChannelTestImpl(ch03, "dummy description", dummy, ValueType.BOOLEAN, loggingInterval,
                                                        loggingTimeOffset);
        LogChannelTestImpl ch4 = new LogChannelTestImpl(ch04, "dummy description", dummy, ValueType.SHORT, loggingInterval,
                                                        loggingTimeOffset);
        LogChannelTestImpl ch5 = new LogChannelTestImpl(ch05, "dummy description", dummy, ValueType.INTEGER, loggingInterval,
                                                        loggingTimeOffset);
        LogChannelTestImpl ch6 = new LogChannelTestImpl(ch06, "dummy description", dummy, ValueType.LONG, loggingInterval,
                                                        loggingTimeOffset);
        LogChannelTestImpl ch7 = new LogChannelTestImpl(ch07, "dummy description", dummy, ValueType.BYTE, loggingInterval,
                                                        loggingTimeOffset);
        LogChannelTestImpl ch8 = new LogChannelTestImpl(ch08, "dummy description", dummy, ValueType.STRING, loggingInterval,
                                                        loggingTimeOffset, valueLength);
        LogChannelTestImpl ch9 = new LogChannelTestImpl(ch09, "dummy description", dummy, ValueType.BYTE_ARRAY, loggingInterval,
                                                        loggingTimeOffset, valueLengthByteArray);
        ...
        for (long i = 0; i < ((60 * 10) * (1000d / loggingInterval)); i++) {
            ...
            LogRecordContainer container1 = new LogRecordContainerImpl(ch01, new Record(new FloatValue(i * -7 - 0.555F), time));
            LogRecordContainer container2 = new LogRecordContainerImpl(ch02, new Record(new DoubleValue(i * +7 - 0.555), time));
            LogRecordContainer container3 = new LogRecordContainerImpl(ch03, new Record(new BooleanValue(boolValue), time));
            LogRecordContainer container4 = new LogRecordContainerImpl(ch04, new Record(new ShortValue((short) i), time));
            LogRecordContainer container5 = new LogRecordContainerImpl(ch05, new Record(new IntValue((int) i), time));
            LogRecordContainer container6 = new LogRecordContainerImpl(ch06, new Record(new LongValue(i * 1000000), time));
            LogRecordContainer container7 = new LogRecordContainerImpl(ch07, new Record(new ByteValue(byteValue), time));
            LogRecordContainer container8 = new LogRecordContainerImpl(ch08, new Record(new StringValue(testString), time));
            LogRecordContainer container9 = new LogRecordContainerImpl(ch09, new Record(new ByteArrayValue(testByteArray), time));
