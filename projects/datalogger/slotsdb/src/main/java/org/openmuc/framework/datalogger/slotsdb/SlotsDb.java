...
public final class SlotsDb implements DataLoggerService {
     ...
     */
    public static String DB_ROOT_FOLDER = System.getProperty(SlotsDb.class.getPackage().getName().toLowerCase() + ".dbfolder");
     ...
     */
    public static String MAX_OPEN_FOLDERS = System.getProperty(SlotsDb.class.getPackage().getName().toLowerCase() + ".max_open_folders");
     ...
     */
    public static String FLUSH_PERIOD = System.getProperty(SlotsDb.class.getPackage().getName().toLowerCase() + ".flushperiod");

    /*
     * configures how long data will at least be stored in the SLOTSDB.
     */
    public static String DATA_LIFETIME_IN_DAYS = System.getProperty(SlotsDb.class.getPackage().getName().toLowerCase() + ".limit_days");

    /*
     * configures the maximum Database Size (in MB).
     */
    public static String MAX_DATABASE_SIZE = System.getProperty(SlotsDb.class.getPackage().getName().toLowerCase() + ".limit_size");
    ...
    protected void activate(ComponentContext context) {
        ...
        if (rootFolder == null) {
            ...
            logger.info("System property " + SlotsDb.class.getPackage().getName()
                                                          .toLowerCase() + ".dbfolder not set. Storing to " + SlotsDb
                    .DEFAULT_DB_ROOT_FOLDER);
        }
    ...
    public void log(List<LogRecordContainer> containers, long timestamp) {
        ...
        for (LogRecordContainer container : containers) {
            ...
            try {
                fileObjectProxy.appendValue(container.getChannelId(), value, timestamp, container.getRecord().getFlag().getCode(),
                                            loggingIntervalsById.get(container.getChannelId()));
            } catch (IOException e) {
